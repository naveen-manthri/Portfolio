    ...
    <Section id="skills" title="My Skills" className="bg-light-surface dark:bg-dark-surface">
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-8 text-center">
        {SKILLS.map((skill, index) => (
          <div 
            key={skill.name} 
            className="flex flex-col items-center justify-center p-4 bg-light-background dark:bg-dark-background rounded-lg shadow-md hover:shadow-xl transform hover:-translate-y-2 transition-all duration-300 opacity-0 animate-fade-in-up"
